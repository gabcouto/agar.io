...
int main(void)
{
	...
	FILE* arquivo;
    	SCORES winners[10];
	int colis[2], saiu_voltou = 0, carregou_ganhadores = 0, naoconta = 1, k, tecla = 0, contaletra = 0, menos;
	char nomejogador[51]={};
	...
	for(j = 0; j < NUM_BOLAS; j++)
	{
		...
		bolas[j].posicao.x = (rand()%2940)+60;
		bolas[j].posicao.y = (rand()%2940)+60;
	    	bolas[j].raio = (rand()%60)+10;
	    	bolas[j].tipo = rand()%3;
	}
				...
				for(j = 0; j<NUM_BOLAS; j++)
				{
					...
					if(IsKeyDown(KEY_RIGHT)){
						for(j=0; j<NUM_BOLAS; j++){
							...
							//fazer uma função para colocar em todas as IsKeyDown() a fim de mexer o restante das bolinhas (enquanto as teclas estão pressionadas)
						}
					}
                    ...
                    for(colis[0]=0; colis[0]<NUM_BOLAS-1; colis[0]++)
					{
						...
						{							
							...
							{
								if(bolas[colis[0]].raio>bolas[colis[1]].raio)
								{
									bolas[colis[1]].posicao.x=rand()%1500;
									bolas[colis[1]].posicao.y=rand()%1500;
									bolas[colis[0]].raio+=(bolas[colis[1]].raio)/10;
									bolas[colis[1]].raio=rand()%30+10;
								}

								if(bolas[colis[0]].raio<bolas[colis[1]].raio)
								{
									bolas[colis[0]].posicao.x=rand()%1500;
									bolas[colis[0]].posicao.y=rand()%1500;
									bolas[colis[1]].raio+=(bolas[colis[1]].raio)/10;
									bolas[colis[0]].raio=rand()%30+10;
								}
