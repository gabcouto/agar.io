...
int main(void)
{
	...
	int colis[2], saiu_voltou = 0, carregou_ganhadores = 0, naoconta = 1, k, tecla = 0, contaletra = 0, j, tempoSemComer = 0, come = 1, posicao_alterabin = 0, entra_rank;
	char nomejogador[51]={};
	...
	while(!WindowShouldClose() && sair == 0)// enquanto nao for detectado o 'ESC', 'X' da janela ou o botao sair nao for apertado
	{
		...
		switch(telaAtual)
		{
			...
			case CARREGAR:
			{
                ...
                fread(&come,sizeof(int),1,arquivo);
                fread(&tempoSemComer,sizeof(int),1,arquivo);
				fclose(arquivo);
			...
			case SALVAR:
			{
				...
				if(IsKeyPressed(KEY_S))
				{
                    ...
                    fwrite(&come,sizeof(int),1,arquivo);
                    fwrite(&tempoSemComer,sizeof(int),1,arquivo);
					fclose(arquivo);
					telaAtual = MENU;
				}
			...
			case SALVAGAN:
			{
				arquivo=fopen("Ganhadores.bin","r+b");
				fread(winners,sizeof(SCORES),10,arquivo);
				...
				{	// pega dados jogador]
					...
					scanf("%s", nomejogador);
					desce_um(winners, entra_rank, 0);
					strcpy(winners[entra_rank].nome, nomejogador);
					winners[entra_rank].score = pontos;
					printf("%d, %d, %s", entra_rank, winners[entra_rank].score, nomejogador);
					rewind(arquivo);
...
{
	if((pontos>winners[n].score)||(n<0))
		return n;
	return entra_ranking(winners, pontos, n-1);
}

void desce_um(pSCORES winners, int n, int i)
{
	if(i>=n)
	{
		winners[i+1].score = winners[i].score;
		strcpy(winners[i+1].nome, winners[i].nome);
		desce_um(winners, n ,i);
